...
def create_my_custom_view():
   cursor.execute('''
        CREATE VIEW IF NOT EXISTS my_custom_view AS
        SELECT users.id, users.name, AVG(grades.grade) AS average_grade
        FROM users
        JOIN grades ON users.id = grades.userid
        GROUP BY users.id, users.name
        HAVING AVG(grades.grade)    ''')
   print("Моё представление создано!")
create_my_custom_view()


def show_my_custom_view():
   cursor.execute('SELECT * FROM more_average_users')
